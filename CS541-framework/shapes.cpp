...
Box::Box()
{
  ...
  //  specularColor = vec3(1.0, 1.0, 1.0);
    ...
    face(I);
    face(rotate(I,  90.0f, vec3(1.0f, 0.0f, 0.0f)));
    face(rotate(I, -90.0f, vec3(1.0f, 0.0f, 0.0f)));
    face(rotate(I,  90.0f, vec3(0.0f, 1.0f, 0.0f)));
    face(rotate(I, -90.0f, vec3(0.0f, 1.0f, 0.0f)));
    face(rotate(I, 180.0f, vec3(1.0f, 0.0f, 0.0f)));

    ComputeSize();
    MakeVAO();
}

void Box::face(const mat4 tr)
{
  int n =  Pnt.size();
  ...
  Quad.push_back(ivec4(n, n+1, n+2, n+3));
}
...
Disk::Disk(const int n)
{
  ...
  //  shininess = 120.0;
    ...
    Tan.push_back(vec3(1.0f, 0.0f, 0.0f));
    ...
    for (int i=0;  i<=n;  i++) {
        ...
        float x = cos(s);
        float y = sin(s);
        Pnt.push_back(vec4(x,y,0.0f,1.0f));
        Nrm.push_back(vec3(0.0f, 0.0f, 1.0f));
        Tex.push_back(vec2(x*0.5+0.5, y*0.5+0.5));
        Tan.push_back(vec3(1.0f, 0.0f, 0.0f));
        if (i>0) {
          Tri.push_back(ivec3(0, i+1, i)); } }
    ComputeSize();
    MakeVAO();
}
   ...
   // shininess = 120.0;
    ...
    for (int i=0;  i<=n;  i++) {
        ...
        for (int j=0;  j<=1;  j++) {
            ...
            float z = t*2.0f - 1.0f;
            Pnt.push_back(vec4(x,y,z,1.0f));
            Nrm.push_back(vec3(x,y, 0.0f));
            Tex.push_back(vec2(s/(2.0*PI), t));
            Tan.push_back(vec3(-sin(s), cos(s), 0.0));
            if (i>0 && j>0) {
                Quad.push_back(ivec4((i-1)*(2) + (j-1),
                                      (i-1)*(2) + (j),
                                      (i  )*(2) + (j),
                                      (i  )*(2) + (j-1))); } } }
    ComputeSize();
...
float ProceduralGround::HeightAt(const float x, const float y)
{
    ...
    float hs = smoothstep(15.0f, 45.0f, l2Norm(vec3(x,y,0)-vec3(highPoint.x,highPoint.y,0)));
